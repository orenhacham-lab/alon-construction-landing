...
הלוגו ממומש כ־**SVG מובנה** ברכיב [`src/components/Logo.tsx`](src/components/Logo.tsx)
(עץ בגווני טורקיז־כחול + שם החברה) — חד בכל רזולוציה ומותאם לרקע בהיר/כהה.
קיים גם קובץ נכס עצמאי ב־[`public/logo.svg`](public/logo.svg).

להחלפה בלוגו הרשמי שלכם: העלו את הקובץ ל־`public/` (למשל `logo.png`) ועדכנו את הרכיב
`Logo.tsx` לשימוש ב־`<img src="/logo.png" .../>`. נשמח לבצע זאת עבורכם.
